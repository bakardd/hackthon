...
import { Sprout, Brain, RefreshCw, Check } from 'lucide-react';
...
interface MLCropRecommendationsProps {
  plot: Plot;
  onCropSelect?: (crop: string) => Promise<void>;
}

export function MLCropRecommendations({ plot, onCropSelect }: MLCropRecommendationsProps) {
  const [predictions, setPredictions] = useState<CropPrediction[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [selectingCrop, setSelectingCrop] = useState<string | null>(null);
  ...
  const handleSelectCrop = async (crop: string) => {
    if (!onCropSelect) return;
    
    setSelectingCrop(crop);
    try {
      await onCropSelect(crop);
      toast({
        title: "Crop Selected",
        description: `${crop} has been set as the current crop for this plot.`,
      });
    } catch (error) {
      toast({
        title: "Error",
          ...
          {predictions.map((prediction, index) => (
            <Card key={index} className={`hover:shadow-lg transition-all ${index === 0 ? 'ring-2 ring-primary/20' : ''}`}>
              ...
              <CardContent className="space-y-4">
                ...
                {onCropSelect && (
                  <Button
                    onClick={() => handleSelectCrop(prediction.crop)}
                    disabled={selectingCrop === prediction.crop || plot.currentCrop?.toLowerCase() === prediction.crop.toLowerCase()}
                    className="w-full"
                    variant={plot.currentCrop?.toLowerCase() === prediction.crop.toLowerCase() ? "outline" : "default"}
                  >
                    {selectingCrop === prediction.crop ? (
                      <>
                        <RefreshCw className="w-4 h-4 mr-2 animate-spin" />
                        Selecting...
                      </>
                    ) : plot.currentCrop?.toLowerCase() === prediction.crop.toLowerCase() ? (
                      <>
