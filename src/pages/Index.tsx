...
const Index = () => {
  ...
  const handlePlotUpdate = async () => {
    ...
    try {
      const updatedPlots = await plotsService.getUserPlots(user.uid);
      setPlots(updatedPlots);
      
      // Update the selected plot with fresh data
      if (selectedPlot) {
        const updatedSelectedPlot = updatedPlots.find(p => p.id === selectedPlot.id);
      ...
      <div className="container mx-auto px-4 py-8">
        <Tabs defaultValue="dashboard" className="space-y-6">
          <TabsList className="grid w-full max-w-2xl mx-auto grid-cols-4 h-auto p-1">
            ...
            <TabsTrigger value="yield" className="gap-2 py-3">
